Create a simple BIM fragment viewer using That Open Engine components. The viewer should load a fragment (use a sample fragment file or create a simple geometric fragment programmatically), display it in a 3D scene with proper lighting and shadows, allow basic camera navigation (orbit, pan, zoom). Use TOE components only (no raw Three.js imports, no manual geometry creation). Output the code as a single TypeScript file with inline comments. Ensure you follow the TOE API Resolution Hierarchy: 1. That Open Engine components (@thatopen/components, @thatopen/ui-components) 2. Three.js via TOE wrappers (OBC.Worlds, OBC.FragmentsManager, etc.) 3. Raw Three.js only when no TOE alternative exists (rare).
...
### Viewpoints (highest)
*Source:* core/Viewpoints.example.ts  
*Package:* core  
```ts
// ...
